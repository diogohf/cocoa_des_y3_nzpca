...
###########################################
## CONVERT DES DATA TO COCOA FORMAT - START
###########################################

def convert_dv_format_from_des_to_cocoa():
    ...
    nz_source_cocoa = [[nz_source_des[j][i] for i in [1,3,4,5,6]] for j in range(1,len(nz_source_des))] 
    nz_lens_cocoa = [[nz_lens_des[j][i] for i in [1,3,4,5,6]] for j in range(1,len(nz_lens_des))] 
...
# convert_dv_format_from_des_to_cocoa()

##########################################
## CONVERT DES DATA TO COCOA FORMAT - END
##########################################
...
def make_dataset():
    with open("./data_realizations_pile3/des_real_pile3.dataset", "r") as ref:
        ...
        for col in range(bins):
            for row in range(lz):
                for j in ["left", "right"]:
                    with open("./data_realizations_pile3/ded_real_bin%d_%s_nz%d_pile3.dataset" % (col,j,row), "w") as f:
                        for line in lines:
