...
:two: **Step 2 of 6**: go to the project folder (`./cocoa/Cocoa/projects`) and clone the Cosmolike LSST-Y1 project:
        ...
        $(cocoapy38) git clone --depth 1 git@github.com:CosmoLike/cocoa_des_y3.git des_y3

The option `--depth 1` prevents git from downloading the entire project history; Cocoa developers should drop the shallow clone option `--depth 1`. By convention, the Cosmolike Organization hosts a Cobaya-Cosmolike project named XXX at `CosmoLike/cocoa_XXX`. However, our scripts and YAML files assume the removal of the `cocoa_` prefix when cloning the repository.
